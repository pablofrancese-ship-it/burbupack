...
function load() { try { const s = localStorage.getItem("bp_v5"); return s ? { ...INIT, ...JSON.parse(s) } : { ...INIT }; } catch { return { ...INIT }; } }
function save(s) { try { localStorage.setItem("bp_v5", JSON.stringify(s)); } catch {} }
...
function getRangoIdx(cantMillares) {
  if (cantMillares <= 2)  return 0;
  if (cantMillares <= 4)  return 1;
  if (cantMillares <= 7)  return 2;
  if (cantMillares <= 10) return 3;
  return 4;
}
...
function calcular(inp, adm) {
  ...
  const costoM2usd = (PESO_M2[burbuja] || 0.0777) * adm.precioPE;
  const costoM2ars = costoM2usd * adm.tipoCambio * factorTipo * factorCapas * factorColor;
  ...
  const rentReal        = costoTotal > 0 ? (utilidad / costoTotal) * 100 : 0;
...
export default function App() {
  const [as, setAsRaw] = useState(load);
  const setAs = fn => setAsRaw(p => { const n = typeof fn === "function" ? fn(p) : fn; save(n); return n; });
  ...
  useEffect(() => {
    setRes(calcular(inp, as));
  }, [inp, as.costoM2usd, as.costoRolloRepegable, as.costoRolloInviolable, as.rentabilidades, as.tipoCambio]);
  ...
  async function fetchDolar() {
    ...
    try { const r = await fetch("https://dolarapi.com/v1/dolares/oficial",{mode:"cors"}); const d = await r.json(); if (d?.venta) setAs(p=>({...p,tipoCambio:d.venta})); } catch {}
...
<tr><td>Cantidad</td><td>${fmtDec(res.cantMillares,1)} millares (${fmt(res.cantUnidades)} unidades)</td></tr>
...
<div class="total">
<div class="lbl">TOTAL — ${fmtDec(res.cantMillares,1)} mil (${fmt(res.cantUnidades)} u.)</div>
...
<div class="footer">Cotización generada por sistema BurbuPack / Empack Inc. — ${today()}</div>
  ...
  const LIMITES = {
    bolsa: {
      Standard:    { anchoMin:10, anchoMax:500, largoMin:10, largoMax:90 },
      Microburbuja:{ anchoMin:10, anchoMax:500, largoMin:10, largoMax:70 },
      Burbujón:    { anchoMin:25, anchoMax:500, largoMin:25, largoMax:70 },
    },
    lamina: {
      Standard:    { anchoMin:10, anchoMax:190, largoMin:10, largoMax:500 },
      Microburbuja:{ anchoMin:10, anchoMax:145, largoMin:10, largoMax:500 },
      Burbujón:    { anchoMin:10, anchoMax:145, largoMin:10, largoMax:500 },
    },
  };

  const usaCinta = inp.cintaRep || inp.cintaInv;
  const solapaNum2 = parseFloat(inp.solapa) || 0;
  const largoNum2  = parseFloat(inp.largo)  || 0;
  ...
  const solapaError = usaCinta && inp.tipo === "bolsa"
    ? solapaNum2 < 4
      ? "Con cinta, la solapa debe ser mínimo 4 cm."
      : largoNum2 > 0 && solapaNum2 > largoNum2 * 0.5
      ? `Con cinta, la solapa no puede superar el 50% del largo (máx. ${largoNum2 * 0.5} cm).`
      : null
    : null;
      ...
      <div style={{ padding:"0 14px" }}>
        ...
        <div style={{ display:"flex", gap:6, marginBottom:14 }}>
          {["cotizador","historial",...(role==="admin"?["admin"]:[])].map(t=>(
            <button key={t} onClick={()=>setTab(t)} style={tBtn(tab===t)}>
              {t==="cotizador"?"Cotizar":t==="historial"?"Cotizaciones":"Admin"}
            </button>
          ))}
        </div>
        ...
        {tab==="cotizador" && (<>
          <div style={crd}>
            ...
            <div style={{ display:"flex", gap:8, marginBottom:12 }}>
              {["bolsa","lamina"].map(t=>(
                <button key={t} onClick={()=>setI("tipo",t)} style={togBtn(inp.tipo===t)}>{t==="bolsa"?"Bolsa":"Lámina"}</button>
              ))}
            </div>
            {/* Burbuja */}
            ...
            <div style={{ display:"flex", gap:8, marginBottom:12 }}>
              {["simple","triple"].map(c=>(
                <button key={c} onClick={()=>setI("capas",c)} style={togBtn(inp.capas===c)}>{c==="simple"?"Simple":"Triple"}</button>
              ))}
            </div>
            {/* Medidas */}
            ...
            {/* Advertencia medidas */}
            {medidaError && (
              <div style={{ background:"#fdf2f2", border:"0.5px solid #e74c3c", borderRadius:10, padding:"10px 14px", marginBottom:12 }}>
                <p style={{ color:"#c0392b", fontSize:13, margin:0, fontWeight:500 }}>⚠️ {medidaError}</p>
              </div>
            )}

            {/* Advertencia solapa con cinta */}
            {solapaError && (
              <div style={{ background:"#fff8e1", border:"0.5px solid #f39c12", borderRadius:10, padding:"10px 14px", marginBottom:12 }}>
                <p style={{ color:"#b7770d", fontSize:13, margin:0, fontWeight:500 }}>⚠️ {solapaError}</p>
              </div>
            )}
            ...
            <div style={crd}>
              ...
              {/* Rentabilidad del pedido — solo admin */}
              {role==="admin" && (
                <div style={{ background:"#1a1a2e", borderRadius:12, padding:"14px 16px", marginBottom:12 }}>
                  ...
                  <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8 }}>
                    <div>
                      <p style={{ fontSize:11, color:"rgba(255,255,255,0.5)", margin:"0 0 3px" }}>Costo Total</p>
                      <p style={{ fontSize:13, fontWeight:700, color:"white", margin:"0 0 2px" }}>${fmt(res.costoTotal)}</p>
                      <p style={{ fontSize:11, color:"#7ecbf5", margin:0 }}>U$S {fmt(res.costoTotalUSD)}</p>
                    </div>
                    <div>
                      <p style={{ fontSize:11, color:"rgba(255,255,255,0.5)", margin:"0 0 3px" }}>Utilidad</p>
                      <p style={{ fontSize:13, fontWeight:700, color:"#4cd964", margin:"0 0 2px" }}>${fmt(res.utilidad)}</p>
                      <p style={{ fontSize:11, color:"#7ecbf5", margin:0 }}>U$S {fmt(res.utilidadUSD)}</p>
                    </div>
                    <div>
                      <p style={{ fontSize:11, color:"rgba(255,255,255,0.5)", margin:"0 0 3px" }}>Rentabilidad</p>
                      <p style={{ fontSize:22, fontWeight:700, color:"#4cd964", margin:0 }}>{res.rentReal}%</p>
                    </div>
                  </div>
                </div>
              )}
            ...
            {(filterV==="todos"?misCot:misCot.filter(c=>c.vendedor===filterV)).length===0
              ...
              : (filterV==="todos"?misCot:misCot.filter(c=>c.vendedor===filterV)).map(c=>(
                <div key={c.id} style={crd}>
                  ...
                  <p style={{ fontSize:13, color:"var(--color-text-secondary)", margin:"0 0 8px" }}>{c.inp.tipo==="bolsa"?"Bolsa":"Lámina"} {c.inp.burbuja} {c.inp.capas} · {fmtDec(c.res.cantMillares,1)} mil u.</p>
                  <div style={{ display:"flex", gap:8 }}>
        ...
        {tab==="admin" && role==="admin" && (<>
          <div style={{ display:"flex", gap:6, marginBottom:14 }}>
            {["costos","vendedores","pines"].map(t=>(
              <button key={t} onClick={()=>setAdminTab(t)} style={tBtn(adminTab===t)}>
                {t==="costos"?"Costos":t==="vendedores"?"Vendedores":"PINs"}
              </button>
            ))}
          </div>
          ...
          {adminTab==="costos" && (<>
            <div style={crd}>
              ...
              <div style={{ display:"flex", gap:8, alignItems:"flex-end" }}>
                <div style={{ flex:1 }}>
                  <label style={lS}>$ ARS por USD</label>
                  <input type="number" value={as.tipoCambio} onChange={e=>setAs(p=>({...p,tipoCambio:parseFloat(e.target.value)||0}))} style={iS}/>
                </div>
                <button onClick={fetchDolar} style={{ padding:"9px 12px", fontSize:13, borderRadius:8, border:`1px solid ${B}`, background:B, color:"white", cursor:"pointer", fontWeight:600, whiteSpace:"nowrap" }}>
                  {dolarLoading?"...":"🔄 Actualizar"}
                </button>
              </div>
            ...
            <div style={crd}>
              ...
              <div style={{ marginBottom:12 }}>
                ...
                {as.costoRolloRepegable>0 && <p style={{ fontSize:11, color:B, margin:"4px 0 0" }}>→ U$S {(as.costoRolloRepegable/(METROS_REP*100)).toFixed(6)} por cm lineal</p>}
              </div>
              <div>
                ...
                {as.costoRolloInviolable>0 && <p style={{ fontSize:11, color:B, margin:"4px 0 0" }}>→ U$S {(as.costoRolloInviolable/(METROS_INV*100)).toFixed(6)} por cm lineal</p>}
              </div>
            </div>
            <div style={crd}>
