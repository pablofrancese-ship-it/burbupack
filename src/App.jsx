...
export default function App() {
  ...
  useEffect(() => {
    ...
    const unsub = onSnapshot(ref, snap => {
      if (snap.exists()) setAsRaw(prev => ({ ...INIT, ...snap.data() }));
  ...
  useEffect(() => { setRes(calcular(inp, as)); }, [inp, as.precioPE, as.costoRolloRepegable, as.costoRolloInviolable, as.rentabilidades, as.tipoCambio]);
...
<div class="footer">Cotización generada por sistema BurbuPack / Empack Inc. — ${today()}</div>
  ...
  if (loading) return (
    <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", minHeight:300, gap:16 }}>
      <BurbuLogo/>
      <p style={{ color:BD, fontSize:14 }}>Cargando...</p>
    </div>
  );
  ...
  const anchoV = parseFloat(inp.ancho)||0, largoV = parseFloat(inp.largo)||0;
  const medidaError = anchoV>0&&(anchoV<lim.anchoMin||anchoV>lim.anchoMax) ? `Ancho debe ser entre ${lim.anchoMin} y ${lim.anchoMax} cm para ${inp.burbuja}.`
    : largoV>0&&(largoV<lim.largoMin||largoV>lim.largoMax) ? `Largo debe ser entre ${lim.largoMin} y ${lim.largoMax} cm para ${inp.burbuja}.` : null;
  ...
  const solapaError = usaCinta&&inp.tipo==="bolsa"
    ...
    : largoV>0&&solapaV>largoV*0.5 ? `Con cinta, la solapa no puede superar el 50% del largo (máx. ${largoV*0.5} cm).` : null : null;
    ...
    <div style={{ fontFamily:"var(--font-sans)", background:"var(--color-background-primary)", minHeight:400 }}>
      ...
      <div style={{ padding:"0 14px" }}>
        ...
        {tab==="cotizador" && (<>
          ...
          {res&&res.error&&<div style={{ ...crd, border:"0.5px solid #e74c3c", background:"#fdf2f2" }}><p style={{ color:"#c0392b", margin:0, fontSize:14 }}>El ancho requerido ({res.anchoReq} cm) supera el ancho útil de la máquina {inp.burbuja} ({res.anchoMaquina} cm).</p></div>}
        </>)}

        {tab==="historial"&&(
          <div>
            {role==="admin"&&<div style={{ marginBottom:12 }}><label style={lS}>Filtrar por vendedor</label><select value={filterV} onChange={e=>setFilterV(e.target.value)} style={iS}><option value="todos">Todos</option><option value="Admin">Admin</option>{as.vendedores.map(v=><option key={v.id} value={v.nombre}>{v.nombre}</option>)}</select></div>}
        ...
        {tab==="admin"&&role==="admin"&&(<>
          <div style={{ display:"flex", gap:6, marginBottom:14 }}>
            {["costos","vendedores","pines"].map(t=>(<button key={t} onClick={()=>setAdminTab(t)} style={tBtn(adminTab===t)}>{t==="costos"?"Costos":t==="vendedores"?"Vendedores":"PINs"}</button>))}
          </div>
          {adminTab==="costos"&&(<>
            <div style={crd}>
